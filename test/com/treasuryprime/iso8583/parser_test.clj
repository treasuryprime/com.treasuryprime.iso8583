(ns com.treasuryprime.iso8583.parser-test
  (:require
   [clojure.string :as str]
   [clojure.test :refer [deftest is testing]]
   [com.treasuryprime.iso8583.binary :as binary]
   [com.treasuryprime.iso8583.format :as format]
   [com.treasuryprime.iso8583.format-iso8583 :as format-iso8583]
   [com.treasuryprime.iso8583.parser :as parser]))
...
(deftest parser-extract-test
  ...
  (testing "Can extract when a variable length field has a codec"
    (let [parser (parser/parser (format/make-field-definitions [[2 :field (format/variable-length-field 3) :codec {:decoder binary/bytes-to-hex}]]))
          msg (parser (binary/hex-to-bytes "3032303040000000000000003132331234"))]
      (is (= (:field msg) "1234")))))
